...
date: "Due date: 11:59 on Sunday, May 31, 2026"
output: 
...
source("../R/generate_partial_clique.R")
source("../R/compute_maximal_partial_clique_2.R")
source("../R/compute_maximal_partial_clique_6.R")
...
- To complete this homework, you may write in the `HW4.Rmd` file.
...
Implementation 2 finds a large subset of nodes in an adjacency matrix whose internal edge density is at least `alpha`. It starts with all nodes sorted by degree, then repeatedly removes the node with the smallest degree within the current subgraph until the remaining nodes are dense enough. It returns the selected node indices and their final edge density. 

Implementation 6 finds a dense subset of nodes in an adjacency matrix whose edge density is at least `alpha`. It starts with all nodes, then repeatedly removes the node whose removal gives the biggest improvement in edge density. The method it calculates the edge density is that it computes the proportion of actual edges among the selected nodes out of all possible edges.
...
```{r test implementation 2, error = TRUE}
...
test_that("compute_maximal_partial_clique outputs a correctly structured list", {
  ...
  output <- compute_maximal_partial_clique_2(adj_mat, alpha = 0.9)
...
test_that("compute_maximal_partial_clique returns values in valid ranges", {
  ...
  output <- compute_maximal_partial_clique_2(adj_mat, alpha = 0.8)
...
test_that("compute_maximal_partial_clique finds a known complete clique", {
  ...
  output <- compute_maximal_partial_clique_2(adj_mat, alpha = 1)
...
test_that("compute_maximal_partial_clique handles corner cases gracefully", {
  ...
  result_small <- compute_maximal_partial_clique_2(
  ...
  result_large <- compute_maximal_partial_clique_2(
  ...
  result_alpha_low <- compute_maximal_partial_clique_2(
  ...
  result_strict <- compute_maximal_partial_clique_2(
    adj_strict,
    alpha = 1.0
  )
  ...
  result_diag <- compute_maximal_partial_clique_2(
    adj_diag,
    alpha = 0.5
  )
  ...
  result_complete <- compute_maximal_partial_clique_2(
    adj_complete,
    alpha = 0.5
  )
...
```{r test implmentation 6, error = TRUE}
# Category 1: Checking that the function outputs the correct type
test_that("compute_maximal_partial_clique outputs a correctly structured list", {
  adj_mat <- matrix(0, nrow = 6, ncol = 6)
  diag(adj_mat) <- 1
  ...
  output <- compute_maximal_partial_clique_6(adj_mat, alpha = 0.9)
...
test_that("compute_maximal_partial_clique returns values in valid ranges", {
  ...
  output <- compute_maximal_partial_clique_6(adj_mat, alpha = 0.8)
...
test_that("compute_maximal_partial_clique finds a known complete clique", {
  ...
  output <- compute_maximal_partial_clique_6(adj_mat, alpha = 1)
...
test_that("compute_maximal_partial_clique handles corner cases gracefully", {
  ...
  result_small <- compute_maximal_partial_clique_6(
    output_small$adj_mat,
    alpha = 1
  )
  ...
  result_large <- compute_maximal_partial_clique_6(
    output_large$adj_mat,
    alpha = 1
  )
  ...
  result_alpha_low <- compute_maximal_partial_clique_6(
    output_alpha_low$adj_mat,
    alpha = 0.5
  )
  ...
  result_strict <- compute_maximal_partial_clique_6(
    adj_strict,
    alpha = 1.0
  )
  ...
  result_diag <- compute_maximal_partial_clique_6(
    adj_diag,
    alpha = 0.5
  )
  ...
  result_complete <- compute_maximal_partial_clique_6(
    adj_complete,
    alpha = 0.5
  )
...
set.seed(10)
result2 <- UWBiost561::compute_maximal_partial_clique_1(
  adj_mat = data$adj_mat,
  alpha = 0.95
)
result2
...
- 3) Which different methods are you using in your simulation study? (This question is moot for this homework since you'll be using all 9 implementations of `compute_maximal_partial_clique`, but please write this down for thoroughness.)
...
- You **<u>must</u>** use all 9 implementations of `compute_maximal_partial_clique` in your simulation study, where each method is used on every adjacency matrix you generate.
...
- Please use `compute_maximal_partial_clique_master()` when using each of the 9 implementations (instead of calling the `compute_maximal_partial_clique` implementations directly). Additionally, please do not set `time_limit` to be larger than 30 (i.e., regardless of how large your simulation study gets, please do not allow more than 1 minute for any implementation). There are 9 implementations, and I wouldn't want just one trial to take more than 5 minutes!
...
Some ideas of what you can test for is: how often does each method get the maximal partial clique (among all 9 methods) when the number of nodes `n` changes, or when `alpha` changes? You can also try incorporating how fast an implementation is (in terms of time) into your simulation study.
...
For my simulation study, I will generate random graphs using my `generate_partial_clique()` function. Each graph will be an undirected adjacency matrix with 1s on the diagonal. The function creates a graph with a planted partial clique: a fraction of the nodes are selected to form a dense subgraph, while the rest of the graph contains lower-density random background edges.

I plan to vary the number of nodes, `n`, across three levels: `n = 10`, `n = 20`, and `n = 30` and keep `clique_fraction = 0.5`, `clique_edge_density = 0.95`, and `alpha = 0.95` fixed. This lets me study how the difficulty of finding a maximal partial clique changes as the graph gets larger.

For each generated adjacency matrix, I will run all 9 implementations of `compute_maximal_partial_clique` using `compute_maximal_partial_clique_master()`. I will use `number = 1, 2, ..., 9` to call the different implementations, and `time_limit = 30` for each method.

I hope to learn which implementations tend to find the largest valid partial cliques, which implementations time out more often, and how performance changes as `n` increases. I expect that larger graphs will be harder, so the methods may take longer or time out more often for larger values of `n`.

I plan to run 3 trials for each level of `n`, so there would be 81 total method runs. Since each method run has a maximum time limit of 30 seconds, the maximum possible runtime is 2430 seconds = 40.5 minutes.
...
Note 5: You (ideally) want to set up your simulation such that for each trial in a level, you generate <u>**one**</u> random adjacency matrix that is used in <u>**all 9**</u> different implementations of `compute_maximal_partial_clique`. This gives a "fair" comparison across all 9 methods. You also might want to set the random seed (via `set.seed()`) before generating that random adjacency matrix for this trial so you can easily reproduce the results from a particular trial if needed.
...
```{r, echo=TRUE, out.width = "400px", fig.align="center", echo = FALSE, fig.cap=c("First screenshot in Question 2E.")}
knitr::include_graphics("figures/check_1.png")
```

```{r, echo=TRUE, out.width = "400px", fig.align="center", echo = FALSE, fig.cap=c("Second screenshot in Question 2E.")}
knitr::include_graphics("figures/check_2.png")
```
...
For my final project, I plan to build a new R package for numerical integration. The package will include functions for approximating definite integrals using methods such as Riemann sums, the trapezoidal rule, and Simpson's rule. I will include examples comparing these numerical approximations to exact integrals for functions where the true answer is known. I plan to make a pkgdown website for this package with documentation and a vignette showing how to use the main functions.
